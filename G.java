...
import javax.swing.*;
import java.sql.*;
class G implements ActionListener,ItemListener
   {    
        JPanel jp;
        JFrame jf;
        JLabel jl1,jl15;
	...
	G()
	  {
		...
		jf.setVisible(true);
             ...
             {
              if((jrb1.isSelected())&&(ae.getSource() == jb))
               {
              ...
              else if((jrb3.isSelected())&&(ae.getSource() == jb))
               {
                int a = Integer.parseInt(jtf1.getText());
                try
                 {
                    ...
                    Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/Hospital_Management" , "root", "ktdf");
                    Statement statement = connection.createStatement();
                    ResultSet rs = statement.executeQuery("select Patient_name,Room_no,Bed_no from Add_Patient where  User_id =("+a+")");
                    while(rs.next())
               ...
               else if((jrb2.isSelected())&&(ae.getSource() == jb))
               {
                int a = Integer.parseInt(jtf1.getText());
                try
                 {
                    ...
                    while(rs.next())
                   jta.setText(" USERNAME"+"   "+rs.getString("User_name")+"   " +"  " +"Department"+"   "+rs.getString("Department")+"   "+ "   "+"Salary"+"    " +rs.getString("Salary"));
                     connection.close();
                  }
               catch(Exception e)
                 { 
